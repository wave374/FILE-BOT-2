...
<pre>{client.reply_text}</pre>
    ...
    reply_markup = InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton('ᴇɴᴅ sᴛɪᴄᴋᴇʀ', 'end_sticker')],
...
@Client.on_callback_query(filters.regex("^end_sticker$"))
async def end_sticker(client, query):
    current = client.messages.get('END_STICKER', '')
    msg = f"""<blockquote>**End Sticker Settings:**</blockquote>
**Current End Sticker:** `{"✓ sᴇᴛ" if current else "✗ ɴᴏᴛ sᴇᴛ"}`

__Tʜɪs sᴛɪᴄᴋᴇʀ ɪs sᴇɴᴛ ᴀꜰᴛᴇʀ ᴀʟʟ ꜰɪʟᴇs ᴀʀᴇ ᴅᴇʟɪᴠᴇʀᴇᴅ ᴛᴏ ᴀ ᴜsᴇʀ, ʙᴇꜰᴏʀᴇ ᴛʜᴇ ᴀᴜᴛᴏ ᴅᴇʟᴇᴛᴇ ᴍᴇssᴀɢᴇ.__

__Usᴇ ᴛʜᴇ ᴀᴘᴘʀᴏᴘʀɪᴀᴛᴇ ʙᴜᴛᴛᴏɴ ʙᴇʟᴏᴡ ᴛᴏ ᴀᴅᴅ ᴏʀ ʀᴇᴍᴏᴠᴇ ᴛʜᴇ ᴇɴᴅ sᴛɪᴄᴋᴇʀ!__
"""
    reply_markup = InlineKeyboardMarkup([
        [
            InlineKeyboardButton(('ꜱᴇᴛ' if not current else 'ᴄʜᴀɴɢᴇ') + '\nᴇɴᴅ sᴛɪᴄᴋᴇʀ', callback_data='add_end_sticker'),
            InlineKeyboardButton('ʀᴇᴍᴏᴠᴇ\nᴇɴᴅ sᴛɪᴄᴋᴇʀ', callback_data='rm_end_sticker')
        ],
...
@Client.on_callback_query(filters.regex("^rm_end_sticker$"))
async def rm_end_sticker(client, query):
    client.messages['END_STICKER'] = ''
    await query.answer('✓ End sticker removed!', show_alert=True)
    await end_sticker(client, query)

#===============================================================#

@Client.on_callback_query(filters.regex("^add_end_sticker$"))
async def add_end_sticker(client, query):
    msg = f"""<blockquote>**Change End Sticker:**</blockquote>
**Current End Sticker:** `{"✓ sᴇᴛ" if client.messages.get('END_STICKER', '') else "✗ ɴᴏᴛ sᴇᴛ"}`

__Sᴇɴᴅ ᴛʜᴇ sᴛɪᴄᴋᴇʀ ʏᴏᴜ ᴡᴀɴᴛ ᴛᴏ sᴇᴛ ᴀs ᴛʜᴇ ᴇɴᴅ sᴛɪᴄᴋᴇʀ, ᴏʀ ᴡᴀɪᴛ ꜰᴏʀ 60 sᴇᴄᴏɴᴅ ᴛɪᴍᴇᴏᴜᴛ ᴛᴏ ʙᴇ ᴄᴏᴍᴘʟᴇᴛᴇᴅ!__
"""
    await query.answer()
    await query.message.edit_text(msg)
    try:
        res = await client.listen(user_id=query.from_user.id, filters=filters.sticker, timeout=60)
        client.messages['END_STICKER'] = res.sticker.file_id
        return await query.message.edit_text("**This sticker has been set as the end sticker!!**", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton('◂ ʙᴀᴄᴋ', 'end_sticker')]]))
    except ListenerTimeout:
        return await query.message.edit_text("**Timeout, try again!**", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton('◂ ʙᴀᴄᴋ', 'end_sticker')]]))
