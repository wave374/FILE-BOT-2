...
async def start_command(client: Client, message: Message):
    ...
    if len(text) > 7:
        ...
        # 8. Send end sticker (after all files, before auto delete message)
        end_sticker = client.messages.get('END_STICKER', '')
        if yugen_msgs and end_sticker:
            try:
                sticker_msg = await client.send_sticker(chat_id=message.from_user.id, sticker=end_sticker)
                yugen_msgs.append(sticker_msg)
            except Exception as e:
                client.LOGGER(__name__, client.name).warning(f"Failed to send end sticker: {e}")
    ...
    # 9. Normal start message
        ...
        buttons = [
    [InlineKeyboardButton("• ᴄʜᴀɴɴᴇʟꜱ •", url="https://t.me/Animeflio_Network/19")],
    [
